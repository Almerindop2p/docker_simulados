...
@section('title', 'Lista de Materias | Painel ADM')
@section('breadcrumb', 'Painel / Materias / Lista')
@section('page_title', 'Lista de Materias')
...
@push('styles')
    <style>
        ...
        th, td { padding: 12px 14px; text-align: left; border-bottom: 1px solid #e8eef7; font-size: 14px; }
        ...
        .icon-btn { width: 34px; height: 34px; border: 1px solid #d2dfef; border-radius: 10px; background: #fff; display: inline-flex; align-items: center; justify-content: center; cursor: pointer; color: #325680; transition: background-color .2s ease, border-color .2s ease, color .2s ease; text-decoration: none; }
        .icon-btn:hover { background: #f4f8ff; border-color: #bfd2ee; color: #1f436f; }
        .icon-btn-delete { color: #a2273c; border-color: #e7ccd2; }
        .icon-btn-delete:hover { background: #fff3f5; border-color: #ddb5bf; color: #8d1f34; }
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-lista-materias">
        <div class="panel-head">
            <div>
                <h2 id="titulo-lista-materias" class="panel-title">Materias cadastradas</h2>
                <p class="panel-subtitle">Gerencie as materias para classificacao e filtros das questoes.</p>
            </div>
            <a class="btn" href="{{ route('adm.materias.create') }}">Adicionar Materia</a>
        </div>
        ...
        @if ($materias->isEmpty())
            <div class="empty">Nenhuma materia cadastrada ate o momento.</div>
        @else
                    ...
                    <tbody>
                        @foreach ($materias as $materia)
                            <tr>
                                ...
                                <td>{{ $materia->name }}</td>
                                <td>{{ $materia->slug }}</td>
                                <td>{{ $materia->created_at?->format('d/m/Y H:i') }}</td>
                                <td>
                                    <div class="row-actions">
                                        <a class="icon-btn" href="{{ route('adm.materias.edit', $materia) }}" title="Editar materia {{ $materia->name }}">
                                        ...
                                        <form method="POST" action="{{ route('adm.materias.destroy', $materia) }}" onsubmit="return confirm('Deseja realmente excluir a materia {{ addslashes($materia->name) }}?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="icon-btn icon-btn-delete" title="Excluir materia {{ $materia->name }}">
