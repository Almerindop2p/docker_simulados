...
@section('title', 'Editar Materia | Painel ADM')
@section('breadcrumb', 'Painel / Materias / Editar')
@section('page_title', 'Editar Materia')
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-form-materia">
        <div>
            <h2 id="titulo-form-materia" class="panel-title">Editar materia</h2>
            <p class="panel-subtitle">Atualize os dados da materia selecionada.</p>
        </div>
        ...
        <form id="materiaForm" class="form-grid" method="POST" action="{{ route('adm.materias.update', $materia) }}" novalidate>
            @csrf
            @method('PUT')
            <div class="field">
                <label class="label" for="name">Nome da materia</label>
                <input id="name" name="name" class="input" type="text" maxlength="120" value="{{ old('name', $materia->name) }}" autocomplete="off" required>
            ...
            <div class="field">
                ...
                <input id="slug" name="slug" class="input" type="text" maxlength="140" value="{{ old('slug', $materia->slug) }}" autocomplete="off" required>
    ...
    <script>
        (function () {
            ...
            function checkField(field, value, statusEl) {
                ...
                return fetch('{{ route('adm.materias.check-field') }}?field=' + encodeURIComponent(field) + '&value=' + encodeURIComponent(trimmed) + '&ignore_id=' + encodeURIComponent(ignoreId), {
