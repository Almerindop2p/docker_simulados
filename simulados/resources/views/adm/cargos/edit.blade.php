...
@section('title', 'Editar Cargo | Painel ADM')
@section('breadcrumb', 'Painel / Cargos / Editar')
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-form-cargo">
        <div>
            <h2 id="titulo-form-cargo" class="panel-title">Editar cargo</h2>
            <p class="panel-subtitle">Atualize os dados do cargo selecionado.</p>
        </div>
        ...
        <form id="cargoForm" class="form-grid" method="POST" action="{{ route('adm.cargos.update', $cargo) }}" novalidate>
            @csrf
            @method('PUT')
            <div class="field">
                <label class="label" for="name">Nome do cargo</label>
                <input id="name" name="name" class="input" type="text" maxlength="120" value="{{ old('name', $cargo->name) }}" autocomplete="off" required>
            ...
            <div class="field">
                ...
                <input id="slug" name="slug" class="input" type="text" maxlength="140" value="{{ old('slug', $cargo->slug) }}" autocomplete="off" required>
            ...
            <div class="actions">
                ...
                <a class="btn btn-soft" href="{{ route('adm.cargos.index') }}">Cancelar</a>
            </div>
        </form>
    </section>
@endsection
    ...
    <script>
        (function () {
            ...
            function checkField(field, value, statusEl) {
                ...
                return fetch('{{ route('adm.cargos.check-field') }}?field=' + encodeURIComponent(field) + '&value=' + encodeURIComponent(trimmed) + '&ignore_id=' + encodeURIComponent(ignoreId), {
