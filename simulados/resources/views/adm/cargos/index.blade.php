...
@section('title', 'Lista de Cargos | Painel ADM')
@section('breadcrumb', 'Painel / Cargos / Lista')
@section('page_title', 'Lista de Cargos')
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-lista-cargos">
        <div class="panel-head">
            <div>
                <h2 id="titulo-lista-cargos" class="panel-title">Cargos cadastrados</h2>
                <p class="panel-subtitle">Gerencie os cargos associados as questoes.</p>
            </div>
            <a class="btn" href="{{ route('adm.cargos.create') }}">Adicionar Cargo</a>
        </div>
        ...
        @if ($cargos->isEmpty())
            <div class="empty">Nenhum cargo cadastrado ate o momento.</div>
        @else
            <div class="table-wrap">
                <table aria-label="Lista de cargos">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Nome do cargo</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="row-actions">
                                        <a class="icon-btn" href="{{ route('adm.cargos.edit', $cargo) }}" title="Editar cargo {{ $cargo->name }}">
                                        ...
                                        <form method="POST" action="{{ route('adm.cargos.destroy', $cargo) }}" onsubmit="return confirm('Deseja realmente excluir o cargo {{ addslashes($cargo->name) }}?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="icon-btn icon-btn-delete" title="Excluir cargo {{ $cargo->name }}">
