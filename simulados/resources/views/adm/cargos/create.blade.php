...
@section('title', 'Adicionar Cargo | Painel ADM')
@section('breadcrumb', 'Painel / Cargos / Adicionar')
@section('page_title', 'Adicionar Cargo')
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-form-cargo">
        <div>
            <h2 id="titulo-form-cargo" class="panel-title">Cadastrar novo cargo</h2>
            <p class="panel-subtitle">Preencha os dados para adicionar um novo cargo ao sistema.</p>
        </div>
        ...
        <form id="cargoForm" class="form-grid" method="POST" action="{{ route('adm.cargos.store') }}" novalidate>
            @csrf
            <div class="field">
            ...
            <div class="actions">
                <button id="submitButton" class="btn btn-primary" type="submit">Cadastrar cargo</button>
    ...
    <script>
        (function () {
            ...
            function checkField(field, value, statusEl) {
                ...
                return fetch('{{ route('adm.cargos.check-field') }}?field=' + encodeURIComponent(field) + '&value=' + encodeURIComponent(trimmed), {
