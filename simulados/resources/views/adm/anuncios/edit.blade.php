...
@section('title', 'Editar Anuncio | Painel ADM')
@section('breadcrumb', 'Painel / Anuncios / Editar')
...
@section('content')
    @php
        ...
        $isActive = old('is_active', (int) ($anuncio->is_active ? 1 : 0)) === 1;
    @endphp

    <section class="panel-card" aria-labelledby="titulo-form-anuncio">
        <div>
            <h2 id="titulo-form-anuncio" class="panel-title">Editar anuncio</h2>
            <p class="panel-subtitle">Atualize formato, status e codigo de exibicao do anuncio selecionado.</p>
        </div>
        ...
        <form id="anuncioForm" class="form-grid" method="POST" action="{{ route('adm.anuncios.update', $anuncio) }}" novalidate>
            @csrf
            @method('PUT')

            <div class="field">
                <label class="label" for="title">Titulo do anuncio</label>
                <input id="title" name="title" class="input" type="text" maxlength="120" value="{{ old('title', $anuncio->title) }}" autocomplete="off" required aria-describedby="title-status">
            ...
            <div class="field">
                ...
                <input id="slug" name="slug" class="input" type="text" maxlength="140" value="{{ old('slug', $anuncio->slug) }}" autocomplete="off" required aria-describedby="slug-status">
            ...
            <div class="field">
                ...
                <select id="format" name="format" class="select" required>
                    ...
                    @foreach ($formatOptions as $value => $label)
                        <option value="{{ $value }}" @selected(old('format', $anuncio->format) === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <fieldset id="adsenseFieldset" class="adsense-fieldset" @if (!$isActive) hidden @endif>
                <div class="field">
                    ...
                    <textarea id="embed_code" name="embed_code" class="code-input" placeholder="<ins class='adsbygoogle' ...></ins>">{{ old('embed_code', $anuncio->embed_code) }}</textarea>
    ...
    <script>
        (function () {
            ...
            function checkField(field, value, statusEl) {
                ...
                return fetch('{{ route('adm.anuncios.check-field') }}?field=' + encodeURIComponent(field) + '&value=' + encodeURIComponent(trimmed) + '&ignore_id=' + encodeURIComponent(ignoreId), {
                    method: 'GET',
                    headers: {
            ...
            slugInput.addEventListener('input', function () {
                ...
                slugTouched = slugInput.value !== toSlug(titleInput.value);
