...
@section('title', 'Editar Banca | Painel ADM')
@section('breadcrumb', 'Painel / Banca / Editar')
@section('page_title', 'Editar Banca')
...
@section('content')
    <section class="panel-card" aria-labelledby="titulo-form-banca">
        <div>
            <h2 id="titulo-form-banca" class="panel-title">Editar banca</h2>
            <p class="panel-subtitle">Atualize os dados da banca selecionada.</p>
        </div>
        ...
        <form id="bancaForm" class="form-grid" method="POST" action="{{ route('adm.bancas.update', $banca) }}" novalidate>
            @csrf
            @method('PUT')
    ...
    <script>
        (function () {
            ...
            function checkField(field, value, statusEl) {
                ...
                return fetch('{{ route('adm.bancas.check-field') }}?field=' + encodeURIComponent(field) + '&value=' + encodeURIComponent(trimmed) + '&ignore_id=' + encodeURIComponent(ignoreId), {
                    method: 'GET',
                    headers: {
